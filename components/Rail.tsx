	...
	return (
		<aside className={`rail rail--${variant}`}>
			{isHome ? (
				<div className="rail__socials" aria-label="Social links">
					{socialLinks.map((link) => (
						<a key={link.label} className="rail__social-link" href={link.href} aria-label={link.label} target="_blank" rel="noreferrer">
							{link.icon}
						</a>
					))}
				</div>
			...
			<span className="rail__meta" aria-label="Copyright 2026">
				<span className="rail__year">2026</span>
				<span className="rail__copyright">&copy;</span>
			</span>
		</aside>
	);
}
